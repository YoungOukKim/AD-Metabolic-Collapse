#!/usr/bin/env Rscript
# =============================================================================
# 77_detection_matched_null.R  -  standalone; paths are relative to the repository root
#
# Settles the detection-matched null for SLC16A3, which currently carries four
# different values across the repository and the manuscript:
#
...
#   SuppTable_detection_null.csv            0.0669       2,117        0.43
#   detection_null.csv                      0.0612       2,042        0.39
#
# Two detection rates are produced, because the manuscript uses both:
#   det_all  over every astrocyte in the 84-donor cohort - the number quoted in
#            the microglia comparison in the Limitations
#   det_bin  over astrocytes in bins 0.2-0.9 only, i.e. the same cells that
#            enter the early-to-late change - the correct denominator for a null
#            matched to that change
#
# No dependencies beyond hdf5r (or rhdf5). Nothing is sourced; the file is
# self-contained. X is read once, in blocks.
#
# Usage (from anywhere):
#   Rscript 77_detection_matched_null.R selftest
#   Rscript 77_detection_matched_null.R
# =============================================================================
