# [CORRECTED] uses neuronal V-ATPase (VATPase_n) and the true donor-level summary.
# =============================================================================
# ED_Fig4_Donor_Partial.R
#
# Extended Data Figure 4 (a-b): Donor-level partial correlation (CPS-adjusted)
#
#   a — Zero-order: MCT4 vs Neuronal V-ATPase (corrected; true donor-level)
#   b — CPS-adjusted partial correlation (corrected; true donor-level)
#
...
donor_clean <- donor_clean[!is.na(donor_clean$MCT4) &
                           !is.na(donor_clean$VATPase_n) &
                           !is.na(donor_clean$mean_cps),]

r0_d <- cor.test(donor_clean$MCT4, donor_clean$VATPase_n)
pc_d <- partial_cor(donor_clean$MCT4, donor_clean$VATPase_n, donor_clean$mean_cps)

ed4a <- ggplot(donor_clean, aes(x = MCT4, y = VATPase_n)) +
  geom_smooth(method="lm", se=TRUE, color="#2166AC") +
  geom_point(size=2, alpha=0.6, color="#2166AC") +
  annotate("text", x=min(donor_clean$MCT4)*1.02, y=max(donor_clean$VATPase_n)*0.95,
           label=sprintf("r = %+.3f\np = %.1e\nn = %d",
