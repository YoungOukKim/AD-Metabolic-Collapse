...
fig5f <- ggplot(ladder_hk, aes(x = step, y = r)) +
  ...
  geom_text(aes(label = sprintf("%+.2f", r)), vjust = -0.4, size = 3.4, fontface = "bold") +
  ylim(0, 0.7) +
  labs(x = "Robustness control", y = "r (HK1 \u2194 MAPT)", title = "f") +
  theme_paper + theme(legend.position = "none")

# ── Panel g: cross-platform validation vs immunoassay (Elecsys) Tau ───────────
if (has_elec) {
...
save_fig(fig5, "Fig5_CSF_Validation.png", width = 11, height = 15)
