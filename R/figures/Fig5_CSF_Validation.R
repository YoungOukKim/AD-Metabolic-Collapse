...
#   d — Progressive partial correlation bar chart (GFAP / TREM2 / NfL control)
...
#
# Input:  ADNI Emory TMT-MS CSF proteomics (requires ADNI data access)
#         Loaded as: data/processed/merged_proteomics_dx.rds
# Output: output/figures/Fig5_CSF_Validation.png
#
# Note: The protein column naming convention is GeneName_UniProtID
#       (e.g., "ATP6V1A_P38606"). Update the P list below if your
#       ADNI dataset uses a different convention.
# =============================================================================
source("R/figures/utils.R")

emory_file <- "data/processed/merged_proteomics_dx.rds"
if (!file.exists(emory_file)) {
  message("ADNI proteomics file not found: ", emory_file)
  message("Access ADNI data at: https://adni.loni.usc.edu")
  stop("ADNI data required for Fig 5")
}
...
# Update these SeqIds/UniProt IDs if your ADNI export differs
P <- list(
  ...
  TFRC = "TFRC_P02786",    FTH1 = "FTH1_P02794",
  LCN2 = "LCN2_P80188",    NEFL = "NEFL_P07196"
)
P <- P[sapply(P, function(x) x %in% names(merged))]

em <- merged[merged$DX %in% c("CN","MCI","DEM"), ]
em$DX <- factor(em$DX, levels = c("CN","MCI","DEM"))
cat(sprintf("ADNI: n=%d  CN=%d MCI=%d DEM=%d\n",
            nrow(em), sum(em$DX=="CN"), sum(em$DX=="MCI"), sum(em$DX=="DEM")))
...
fig5c <- ggplot(scat, aes(x = .data[[P$V1A]], y = .data[[P$MAPT]], color = DX)) +
  ...
  annotate("text", x = min(scat[[P$V1A]], na.rm=TRUE),
           y = mapt_clip * 0.90,
           label = sprintf("r = +%.3f\nn = %d", r_c, nrow(scat)),
...
pc_bar <- data.frame(
  Control = factor(c("None","| GFAP","| GFAP+TREM2","| GFAP+NfL","| GFAP+TREM2+NfL"),
...
fig5e <- ggplot(hk_dat, aes(x = .data[[P$HK1]], y = .data[[P$MAPT]], color = DX)) +
  ...
  annotate("text", x = min(hk_dat[[P$HK1]], na.rm=TRUE),
           y = mapt_clip2 * 0.90,
