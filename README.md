...
| Per-subject rows of the `Fig5_data` sheet | ADNI individual-level data keyed by RID. The repository copy of the source-data workbook is `Supporting_Data_Values_v9.xlsx`, with that sheet replaced by a pointer; every aggregate sheet is intact |
...
| Supporting Data Values | `output/revision2/Supporting_Data_Values_v9.xlsx` (per-subject ADNI sheet removed; see above) |
